...
def frame_to_payload(frame: Frame) -> RadarPayload:
    ...
    target = TargetState(
        ...
        range_m=round(estimate.range_m, 1),
...
class SimulationService:
    ...
    async def next_frame(self) -> Frame:
        ...
        rssi_slope = (
            (obs.rssi_dbfs - prev_rssi) / self._config.simulation.dt_s
...
class ReplayFrameSource:
    ...
    def _emit_at(self, index: int) -> Frame:
        ...
        prev_rssi = self._prev_rssi
        self._prev_rssi = observation.rssi_dbfs
        rssi_slope = (
            (observation.rssi_dbfs - prev_rssi) / self._config.simulation.dt_s
            if prev_rssi is not None
            else 0.0
        )
